...
public class TopicTrainer {
	...
	static CommandOption.String betaFile = new CommandOption.String(TopicTrainer.class, "beta-file", "FILENAME", true, "betafile.csv",
		 "Beta Path parameter: provide a prior of beta param which is a smoothing parameter for each topic-word. beta_w = [this value]",null);
	...
	public static void main (String[] args) throws java.io.IOException {
		...
		if (inputModelFilename.value != null) {
			...
			LabelAlphabet topicAlphabet = newLabelAlphabet(numTopics.value);
			topicModel = new ParallelTopicModel (topicAlphabet, alpha.value, betaFile.value);
		}
		...
		if (inputFile.value != null) {
			...
			try {
				if (inputFile.value.startsWith("db:")) {
					...
					System.out.println("Number of types (vocab size): " + training.getDataAlphabet().size());
