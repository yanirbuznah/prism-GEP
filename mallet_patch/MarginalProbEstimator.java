...
public class MarginalProbEstimator implements Serializable {
	...
	protected int numTypes; // Number of types in the vocabulary
	...
	public MarginalProbEstimator (int numTopics, int numTypes,
		...
		for (int limit = 0; limit < docLength; limit++) {
			...
			if (usingResampling) {
				...
				for (int position = 0; position < limit; position++) {
					...
					double sample = random.nextUniform() * (typeSmoothingMass + typeTopicBetaMass + topicTermMass);
					...
					if (newTopic == -1) {
						System.err.println("sampling error: "+ origSample + " " + sample + " " + typeSmoothingMass + " " + 
										   typeTopicBetaMass + " " + topicTermMass);
			...
			*/
			...
			wordProbabilities[limit] +=
				(typeSmoothingMass + typeTopicBetaMass + topicTermMass) /
				(alphaSum + tokensSoFar);
    ...
    private void writeObject (ObjectOutputStream out) throws IOException {
        ...
        out.writeObject(smoothingOnlyMass);
	...
	private void readObject (ObjectInputStream in) throws IOException, ClassNotFoundException {
        ...
        smoothingOnlyMass = (double[]) in.readObject();
